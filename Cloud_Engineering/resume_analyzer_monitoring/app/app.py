...
def home():
    ...
    filename = session.get('resume_filename', None)
    return render_template("index.html", resume_filename=filename)
...
def analyze():
    ...
    try:
        ...
        if resume_file and resume_file.filename != '' and allowed_file(resume_file.filename):
            # Store filename in session for persistence
            ...
            # Clear file from session if no file uploaded
